...
suite ut_param_tests = [] {
    "parameterized"_test = [] {
        for (auto a : std::vector{1, 2, 3})
        {
            test("arg:"s + std::to_string(a)) = [a] { expect(that % a > 0) << "all are greater than 0"; };
        }

        "altrnative"_test = [](const auto &a) { expect(that % a > 0) << "all values are greater than 0"; } |
                            std::vector{1, 2, 3};
...
suite ut_skip_tests = [] {
    "skipping"_test = [] { skip / "skip this test"_test = [] { expect(42_i == 43) << "should not fire"; }; };
    "tagging"_test = [] {
...
suite ut_matcher = [] {
    "matcher"_test = [] {
        "42_is_between_1_100"_test = [] { expect(match::is_between(1, 100)(42)); };

        "0_is_not_between_1_100"_test = [] { expect(not match::is_between(1, 100)(0)); };
    };
};
